      ...
      <table className="w-full">
        <thead>
          <tr className="bg-gray-50 text-xs text-gray-500 uppercase tracking-wide">
            ...
            <th className="text-left px-5 py-3">Produits</th>
        ...
        <tbody className="divide-y divide-gray-50">
          {orders.map((order) => {
            const productsFull = order.items.map((i) => `${i.quantity}× ${i.name}`).join(', ');
            const productsShort = (() => {
            ...
            return (
              <tr key={order.id} className="hover:bg-gray-50 transition">
                ...
                <td className="px-5 py-4 text-sm text-gray-600 max-w-xs">
                  <span className="block truncate" title={productsFull}>
                    {productsShort}
                  </span>
                </td>
                ...
                <td className="px-5 py-4 text-right text-sm font-bold text-secondary-900">
